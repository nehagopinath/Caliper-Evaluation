...
class Model extends Contract {
    ...
    async addModel(ctx,modelNumber,modelName, feature) {
    ...
    async addInitialGradients(ctx,modelNumber,modelName, worker1, worker2) {
        console.info('============= START : Add initial gradients ===========');
        ...
        await ctx.stub.putState(modelNumber, Buffer.from(JSON.stringify(added_model)));
        console.info('============= END : Add initial gradients ===========');
    }


    async addInitialGradientsTest(ctx,modelNumber,modelName, gradientValue) {
        console.info('============= START : Add initial gradients Test ===========');
        ...
        await ctx.stub.putState(modelNumber, Buffer.from(JSON.stringify(added_model)));
        console.info('============= END : Add initial gradients Test ===========');
    }
    ...
    async queryAllModels(ctx, startKey, endKey) {
    ...
    async sendGradients(ctx, modelNumber, newValue) {
        console.info('============= START : sendGradients ===========');
        ...
        model.gradientValue = newValue;

        await ctx.stub.putState(modelNumber, Buffer.from(JSON.stringify(model)));

        console.info('============= END : sendGradients ===========');
   ...
   /* async mergeAndUpdate(ctx) {
        ...
        var gradient1 = await this.getGradients(ctx, 'Client87510_MODEL5', 'worker1');
        var gradient2 = await this.getGradients(ctx, 'Client87510_MODEL5', 'worker2');
        ...
        var weight = await this.getWeightMatrix(ctx, 'Client87510_MODEL3');
        ...
        await this.updateWeightMatrix(ctx, 'Client87510_MODEL3', updatedWeightMatrix.toString());
    ...
    async mergeAndUpdateTest(ctx, gradient, weight, modelNumber) {

        console.info('============= START : mergeAndUpdateTest ===========');

        var strArr = gradient.toString().split(',').map(Number);
        console.log(`Gradients from worker 1 array: ${strArr}`);
        ...
        await this.updateWeightMatrix(ctx, modelNumber, updatedWeightMatrix.toString());

        console.info('============= END : mergeAndUpdateTest ===========');
